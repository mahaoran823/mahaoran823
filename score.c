...
SS* readDataFromFile(int *N)
{
	...
	while ((!feof(fp)))
	{
		...
		//输出排序后的学生信息
		printf("* 学号：%s	姓名:%s		平时成绩：%4.2f分	实验成绩：%4.2f分\n 	期末成绩:%4.2f分\n", (stu[index].number), (stu[index].name), stu[index].dailyScore, stu[index].shiyanScore, stu[index].finalScore);
...
}void chaxun(SS stu[], int N)
{
	...
	scanf("%s",xuehao);
		for (i = 0; i < N; i++)
		{
			if (strcmp(stu[i].number, xuehao) == 0)
				printf("学号：%s	姓名 : %s		平时成绩：%4.2f分	实验成绩：%4.2f分\n 	期末成绩 : %4.2f分\n", (stu[i].number), (stu[i].name), stu[i].dailyScore, stu[i].shiyanScore, stu[i].finalScore);
		}
}
float fenxi(SS stu[], int N)
{
	int i, sum = 0, m = 0;
	float ave, fangcha;
	for (i = 0; i < N; i++)
	...
	for (i = 0; i < N; i++)
	{
		m += (stu[i].generalScore - ave)*(stu[i].generalScore - ave);
	}
	fangcha = m / N;
	printf("平均成绩：%f,方差：%f", ave, fangcha);
}
